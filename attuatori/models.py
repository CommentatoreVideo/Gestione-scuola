...
class Compito(models.Model):
  ...
  def modifica(self,nuovaSequenza):
    elencoNuovo=nuovaSequenza.split(",")
    vecchioElenco=self.elenco.split("*")
    print(vecchioElenco)
    print(elencoNuovo)
    offset=1
    for i in range(len(elencoNuovo)):
      vecchioElenco[i+offset]=elencoNuovo[i]
      offset+=1
    stringa=""
    for i in range(len(vecchioElenco)):
      stringa+=str(vecchioElenco[i])
      if(i!=len(vecchioElenco)-1):
        stringa+="*"
